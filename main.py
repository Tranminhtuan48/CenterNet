...
def train(args, params):
    ...
    filenames = []
    for filename in os.listdir('/Users/trandungcuong/Desktop/Intern/CenterNet/vehicle/images/train/'):
        filenames.append('/Users/trandungcuong/Desktop/Intern/CenterNet/vehicle/images/train/' + filename)
    ...
    loader = data.DataLoader(dataset, args.batch_size, sampler is None, sampler, num_workers=8, pin_memory=True, collate_fn=Dataset.collate_fn1)

    if args.world_size > 1:
        # DDP mode
        model = torch.nn.parallel.DistributedDataParallel(module=model, device_ids=[args.local_rank], output_device=args.local_rank)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (samples, targets) in p_bar:
            x = i + num_batch * epoch  # number iterations
...
def test(args, model=None):
    filenames = []
    for filename in os.listdir('/Users/trandungcuong/Desktop/Intern/CenterNet/vehicle/images/val/'):
        filenames.append('/Users/trandungcuong/Desktop/Intern/CenterNet/vehicle/images/val/' + filename)

    dataset = Dataset(filenames, args.input_size, False)
    loader = data.DataLoader(dataset, 4, False, num_workers=4, pin_memory=True, collate_fn=Dataset.collate_fn2)
    ...
    # Config
    iou_v = torch.linspace(0.5, 0.95, 10).cuda()  # iou vector -> mAP@0.5:0.95
    n_iou = iou_v.numel()
    ...
    for samples, targets in p_bar:
        ...
        samples = samples.half()  # uint8 -> fp16/32
        samples = samples / 255  # 0 - 255 -> 0.0 - 1.0
        ...
        for si, output in enumerate(outputs):
            ...
            nl, npr = labels.shape[0], output.shape[0]
