...
def resize(image, input_size):
    ...
    image = cv2.copyMakeBorder(image, top, bottom, left, right, cv2.BORDER_CONSTANT)  # border
...
def mix_up(image1, label1, image2, label2):
    # MixUp augmentation (Nguồn: https://arxiv.org/pdf/1710.09412.pdf)
    r = numpy.random.beta(32.0, 32.0)  # alpha=beta=32.0
...
def smooth(y, f=0.05):
    ...
    nf = round(len(y) * f * 2) // 2 + 1  # number of filter (odd)
...
def compute_ap(recall, precision):
    ...
    if method == 'interp':
        ...
        i = numpy.where(m_rec[1:] != m_rec[:-1])[0]  # x axis (recall) changes points
...
class EMA:
    # Updated Exponential Moving Average (EMA) https://github.com/rwightman/pytorch-image-models

    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        # Create EMA
        self.ema = copy.deepcopy(model).eval()  # FP32 EMA
        self.updates = updates  # number of EMA updates
        self.decay = lambda x: decay * (1 - math.exp(-x / tau))  # decay exponential ramp (early epochs)
...
def gaussian_target(heatmap, center, radius, k=1):
    ...
    torch.max(masked_heatmap,
              masked_gaussian * k, out=out_heatmap[y - top:y + bottom, x - left:x + right])

    return out_heatmap


def gaussian_radius(det_size, min_overlap):
    # Generate 2D gaussian radius. Returns: radius (int): Radius of gaussian kernel.
...
def transpose_and_gather(x, indices):
    #  Transpose & gather feature [index]
    #  Args:
...
class ComputeLoss(torch.nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # Vì kênh của wh_target và offset_target là 2 nên hệ số trung bình của loss_center luôn bằng 1/2 của loss_wh và loss_offset.
        loss_center = self.loss_center(outputs[0], targets[0], average_factor=average_factor)
...
class Decoder:
    ...
    def __call__(self, center, wh, offset, input_size):
        ...
        for i in range(center.shape[0]):
            ...
            batch_det_bboxes, batch_labels = self.decode(x, y, z, input_size, k=100, kernel=3)
    ...
    def decode(self, center, wh, offset, input_size, k=100, kernel=3):
